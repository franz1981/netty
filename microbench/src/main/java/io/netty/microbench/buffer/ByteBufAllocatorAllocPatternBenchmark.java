...
public class ByteBufAllocatorAllocPatternBenchmark extends AbstractMicrobenchmark {
    ...
    public void setupAllocatorAndPollute(BenchmarkParams params) throws InterruptedException {
        ...
        this.buffers = new ByteBuf[MAX_LIVE_BUFFERS];
        ...
        Runnable pollutionTask = () -> {
            ...
            if (directAllocation) {
                for (int i = 0; i < perThreadIterations; i++) {
                    performDirectAllocation(allocator, pollutionBuffers);
                }
            } else {
                for (int i = 0; i < perThreadIterations; i++) {
                    performHeapAllocation(allocator, pollutionBuffers);
                }
            }
    ...
    private void performDirectAllocation(ByteBufAllocator alloc, ByteBuf[] buffers) {
    ...
    @CompilerControl(CompilerControl.Mode.DONT_INLINE)
    private void performHeapAllocation(ByteBufAllocator alloc, ByteBuf[] buffers) {
        int size = sizes[getNextSizeIndex()];
        int releaseIndex = getNextReleaseIndex();
        ByteBuf oldBuf = buffers[releaseIndex];
        if (oldBuf != null) {
    ...
    public void directAllocation() {
        performDirectAllocation(allocator, buffers);
    }

    @Benchmark
    public void heapAllocation() {
        performHeapAllocation(allocator, buffers);
    }
